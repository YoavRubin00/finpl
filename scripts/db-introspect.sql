 ...
 * Returns a single column `metadata_json_to_import` (a JSON string) describing every
 * table, column, primary key, foreign key, index, view and check constraint.
 * Read-only — schema metadata only, never table rows.
 *
 * Run by scripts/db-diagram.mjs (`npm run db:diagram`), which feeds the JSON into our
 * own React Flow renderer (scripts/db-diagram.template.html).
 *
 * Query adapted (standard PostgreSQL edition) from ChartDB's open-source introspection
 * script: https://github.com/chartdb/chartdb/blob/main/src/lib/data/import-metadata/scripts/postgres-script.ts
 */
WITH fk_info AS (
