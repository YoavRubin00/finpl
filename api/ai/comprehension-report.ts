...
סיימת בדיוק שיחת בדיקת-הבנה קולית קצרה עם משתמש על המודולה "${title}".
תפקידך: לתת דוח-הבנה הוגן ומעודד, בעברית, בקול שלך — חם, ישיר, בלי להתנשא.
חוקים: אל תמציא. דרג אך ורק לפי מה שהמשתמש באמת אמר בתמלול ולפי הביצועים. אם המשתמש כמעט לא דיבר — ציון נמוך אבל נסח את זה בעידוד. בלי אימוג'ים, בלי המילה "ז'רגון", בלי אנגלית מיותרת. דבר אל המשתמש בגוף שני יחיד.
החזר JSON תקין בלבד בסכמה:
{"understandingScore": <0-100>, "verdictHe": "<משפט אחד חם שמסכם כמה הבין>", "strengthsHe": ["<מה תפס טוב>", ...], "improvementsHe": ["<על מה כדאי לחזור, ספציפי>", ...], "perConcept": [{"concept": "<שם המושג>", "graspPct": <0-100>}, ...]}
strengthsHe ו-improvementsHe: 1-3 פריטים כל אחד, קצרים. perConcept: שורה לכל מושג מהרשימה.`;
...
הפק עכשיו את דוח-ההבנה כ-JSON לפי הסכמה.`;
    ...
    const report: ComprehensionReport = {
      ...
      verdictHe:
        ...
        'סיימנו את הבדיקה — תכף נראה איפה אתה עומד.',
