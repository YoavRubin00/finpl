...
export function ClanMembersTab(): React.ReactElement {
  ...
  const sortedMembers = [...MOCK_MEMBERS].map((m) =>
    m.id === SELF_ID ? { ...m, reputation: selfReputation } : m
  ).sort((a, b) => {
    ...
    <View style={{ flex: 1 }}>
      ...
      <View style={{ flexDirection: 'row-reverse', gap: 8, paddingHorizontal: 16, paddingVertical: 10 }}>
        {([
          ...
          >
            <Text style={{ fontSize: 12, fontWeight: '700', color: sortKey === key ? '#0a1628' : 'rgba(255,255,255,0.7)' }}>
              {label}
            </Text>
          </Pressable>
        ))}
            ...
            {selectedMember && (
              ...
              >
                ...
                <View style={{ alignItems: 'center', gap: 8 }}>
                  ...
                  <Text style={{ fontSize: 20, fontWeight: '900', color: '#ffffff', writingDirection: 'rtl' }}>
                    {selectedMember.name}
                  </Text>
                  <View style={{ backgroundColor: 'rgba(255,255,255,0.1)', borderRadius: 10, paddingHorizontal: 10, paddingVertical: 3 }}>
                    <Text style={{ fontSize: 12, color: CLAN.tierGold, fontWeight: '700' }}>
                      {selectedMember.role === 'leader' ? S.roleLeader : selectedMember.role === 'deputy' ? S.roleDeputy : S.roleMember}
                    </Text>
                  </View>
                </View>
                ...
                <View style={{ flexDirection: 'row-reverse', justifyContent: 'space-around' }}>
                  {[
                    { label: 'XP', value: selectedMember.xp.toLocaleString('he-IL'), color: '#a78bfa' },
                    { label: '🪙', value: selectedMember.coins.toLocaleString('he-IL'), color: CLAN.tierGold },
                    { label: 'מוניטין', value: String(selectedMember.reputation), color: '#4ade80' },
                  ].map((stat) => (
                    <View key={stat.label} style={{ alignItems: 'center' }}>
                      <Text style={{ fontSize: 18, fontWeight: '900', color: stat.color }}>{stat.value}</Text>
                      <Text style={{ fontSize: 11, color: 'rgba(255,255,255,0.5)', writingDirection: 'rtl' }}>{stat.label}</Text>
                    </View>
                  ))}
                </View>
                ...
                <Pressable onPress={() => setSelectedMember(null)}>
                  <Text style={{ textAlign: 'center', color: 'rgba(255,255,255,0.4)', fontSize: 14, paddingVertical: 4 }}>
                    סגור
                  </Text>
                </Pressable>
              </View>
