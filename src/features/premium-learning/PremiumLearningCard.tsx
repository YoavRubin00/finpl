...
function TapZonesCard({ item, onStartModule }: { item: PremiumLearningModule; onStartModule: () => void }) {
...
interface Props {
  item: PremiumLearningModule;
  isActive: boolean;
  /**
   * When set, the final-step CTA invokes this callback instead of navigating
   * to the lesson route. Used when this card renders inside LessonFlowScreen.
   */
  onContinue?: () => void;
}

export const PremiumLearningCard = React.memo(function PremiumLearningCard({ item, isActive: _isActive, onContinue }: Props) {
